...
struct NewsDetail: View {
    ...
    @State private var webHeight: CGFloat = .zero
   ...
   // let detailBaseURL = "https://www.punjabkingsipl.in/apiv3/article/pbks-vs-lsg-punjab-kings-surge-towards-playoffs-with-commanding-win?is_app=1"
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            VStack {
                ...
                let imagePath = viewModel.newsDetail?.content?.data?.imagePath ?? ""
                let imageName = viewModel.newsDetail?.content?.data?.imageFileName ?? ""
                let url = constructImageURL(baseURL: baseURL, imagePath: imagePath, imageName: imageName)
                ...
                HtmlWebView(htmlString: htmlString, contentHeight: $webHeight)
